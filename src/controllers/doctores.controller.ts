...
export const getById = async (
  ...
  if (id.length < 24) {
    return res
      .status(400)
      .json({ code: "404", message: "URL sin parametros para buscar." });
  } else {
    ...
    if (user) {
      ...
      return res.status(404).json({
        code: "404",
        message: "Usuario no fue encontrado o no existe.",
      });
    }
  }
};
...
export const getSearch = async (
  ...
  user.forEach((x, index) => {
    if (
      x.genero == genero ||
      x.tarifa_g == max ||
      x.especialidades.especialidad[0].nombre === especialidad ||
      x.rating.toFixed() === calificacion
    ) {
      data.push(x)
    }
  });
